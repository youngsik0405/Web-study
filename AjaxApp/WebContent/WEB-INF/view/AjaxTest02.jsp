...
<script type="text/javascript">
	...
	function sum()
	{
		...
		//   		   (POST 사용 가능. 단, POST 사용시 헤더 추가 필요)
		//	  『url』: AJAX 요청 페이지 
		//			   (데이터 수신 및 응답 페이지)
		//	  『true』: boolean
		//				true → 비등기, false → 동기, 동기는 사용 안함.
		//-- 실제 서버와의 연결이 아니고
		//   일종의 환경 설정 부분으로 이해하는 것이 바람직하다.
		//   (실제 서버 연결은 나중에...)
		
		
		// ajax 객체(→ XMLHttpRequest 객체)의 readyState 속성은
		// 각 단계별로 변화하며, 0 부터 4까지 순차적으로 증가하는 형태로 변화한다.
		// 그 때 마다... 『readystatchange』 이벤트가 발생한다.
		ajax.onreadystatechange = function()
		{
			// readystatechange 이벤트가 발생한 상황에서...
			// readyState 속성을 확인하여 그 중에서... (0 부터 4까지의 상태 중에서...)
			// 4 가 되는 순간이 요청됐던 서버로부터 응답이 완료된 시점이기 때문에
