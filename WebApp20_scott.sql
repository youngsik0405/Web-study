...
VALUES(1, '공찬민', '1234', 'kcm@test.com', '작성테스트', '게시물 내용 작성', '211.234.142.163', 0, SYSDATE);
--> 한 줄 구성
INSERT INTO TBL_BOARD(NUM, NAME, PWD, EMAIL, SUBJECT, CONTENT, IPADDR, HITCOUNT, CREATED) VALUES(1, '공찬민', '1234', 'kcm@test.com', '작성테스트', '게시물 내용 작성', '211.234.142.163', 0, SYSDATE)
;
--==>> 1 행 이(가) 삽입되었습니다.
...
SELECT *
FROM TBL_BOARD;
--==>> 1	공찬민	1234	kcm@test.com	작성테스트	게시물 내용 작성	211.234.142.163	0	2025-01-16
...
(
    ...
    FROM 
    (
        SELECT NUM, NAME, SUBJECT, HITCOUNT, TO_CHAR(CREATED, 'YY-MM-DD') AS CREATED
        FROM TBL_BOARD
        ORDER BY NUM DESC
    ) DATA
)
...
--> 한 줄 구성
SELECT NUM, NAME, SUBJECT, HITCOUNT, CREATED FROM ( SELECT ROWNUM RNUM, DATA.* FROM ( SELECT NUM, NAME, SUBJECT, HITCOUNT, TO_CHAR(CREATED, 'YY-MM-DD') AS CREATED FROM TBL_BOARD ORDER BY NUM DESC ) DATA ) WHERE RNUM>=1 AND RNUM<=10
;
...
SELECT NUM, NAME, PWD, EMAIL, SUBJECT, CONTENT, IPADDR, HITCOUNT
     , TO_CHAR(CREATED, 'YY-MM-DD')
FROM TBL_BOARD
WHERE NUM = 1;
--> 한 줄 구성
SELECT NUM, NAME, PWD, EMAIL, SUBJECT, CONTENT, IPADDR, HITCOUNT, TO_CHAR(CREATED, 'YY-MM-DD') FROM TBL_BOARD WHERE NUM = 1
;
