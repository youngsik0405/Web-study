	...
	※ 요청한 페이지(jsptest003.jsp)의 결과를 확인한 후
	   ...
	   - 파일
	     ...
	     ==> 변수 a 는 클래스(jsptest003.jsp)의 전역변수로 선언된 것을 확인할 수 있다.
	     	 ...
	     	 변수 b 는 메소드(jsptest003.jsp 클래스의 _jspServlet())의 지역변수로 선언된 것을 확인할 수 있다.
	     	 
	     	 『_jspInit()』 메소드와 『_jspDestroy()』 메소드는
	     	 서비스가 시작될 때와 끝날 때 한 번씩만 실행되는(호출되는) 메소드이다.
	     	 반면... 『_jspServlet()』 메소드는
	     	 브라우저에 의해 페이지가 요청될 때 마다 매번 실행되는(호출되는) 메소드이다.
	     	 
	     	 이로 인해 변수 b 는 페이지에 대한 요청이 발생할 때 마다
	     	 새롭게 초기화 되는 과정을 거치게 되는 것이다.
	     	 
	     	 JSP 페이지의 스크립릿 영역에서 선언된 모든 변수는
	     	 이 『_jspServlet()』메소드의 지역변수가 된다.
	     	 
	     	 또한, JSP 페이지의 선언부 영역에서 선언된 모든 변수는
	     	 클래스의 전역변수(인스턴스 변수)가 된다.
	     	 
	     	 JSP 페이지의 메소드 정의 시
	         스크립 릿 영역 내부에서는 정의할 수 없다.
	       	 서블릿으로 변환되는 과정에서 메소드 내부에 또 다른 메소드가 정의되는 상황이
